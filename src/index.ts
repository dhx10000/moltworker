...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  if (!hasCloudflareGateway && !hasLegacyGateway && !hasOpenAIKey) {
    missing.push(
      'OPENAI_API_KEY, or CLOUDFLARE_AI_GATEWAY_API_KEY + CF_AI_GATEWAY_ACCOUNT_ID + CF_AI_GATEWAY_GATEWAY_ID',
    );
  }
